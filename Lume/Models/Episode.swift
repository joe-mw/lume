...
final class Episode {
    ...
    // scanning every episode in the catalog. `downloadStatusRaw` backs the
    // Downloads screen's live `@Query`, which re-evaluates during catalog syncs.
    #Index<Episode>([\.isWatched], [\.watchProgress], [\.downloadStatusRaw])
